...
const HotelDashboard = () => {
  ...
  const generateWeeklyData = () => {
    ...
    // Instead of starting with Sunday, use the exact start date
    // to ensure we don't have weeks starting before the selected range
    const firstDay = new Date(startDate);
    
    // Generate week boundaries starting from the selected start date
    const weekBoundaries = [];
    let currentDate = new Date(firstDay);
    
    // Process weeks until we reach the end date
    while (currentDate <= endDate) {
      // Find the end of the current week (either 6 days later or the end date, whichever is earlier)
      const weekEnd = new Date(currentDate);
      weekEnd.setDate(currentDate.getDate() + 6);
      
      // Cap the week end at the overall end date
      const actualWeekEnd = new Date(Math.min(weekEnd.getTime(), endDate.getTime()));
      
      // Format dates for display
      const weekStartStr = formatDate(currentDate);
      const weekEndStr = formatDate(actualWeekEnd);
      
      // Calculate days in this week range
      const daysInWeek = getDaysInRange(startDate, endDate, currentDate, actualWeekEnd);
      
      // Only add weeks that have at least one day in the selected range
      if (daysInWeek > 0) {
        const weekLabel = daysInWeek < 7 ? 
          `${weekStartStr} - ${weekEndStr} (${daysInWeek} days)` : 
    ...
    // Sort weeks chronologically (should already be in order, but making it explicit)
    weekBoundaries.sort((a, b) => a.start - b.start);
    ...
    for (const week of weekBoundaries) {
      ...
      const avgUnavailableRoomsPerDay = week.daysInRange > 0 ? totalUnavailableRoomNights / week.daysInRange : 0;
      const adjustedAvailableRoomNights = (roomsPerType - avgUnavailableRoomsPerDay) * week.daysInRange;
  ...
  const sortedData = useMemo(() => {
    ...
    return [...data].sort((a, b) => {
      ...
      // Special case for occupiedRooms which maps to soldRooms in the data
      if (sortColumn === 'occupiedRooms') {
        valueA = a['soldRooms'];
        valueB = b['soldRooms'];
      }
      
      // Convert percentage strings, currency strings, or number strings to numbers for sorting
      ...
      if (typeof valueB === 'string') {
        ...
        } else if (!isNaN(parseFloat(valueB))) {
          valueB = parseFloat(valueB);
        }
      }
      
      // For date columns in daily view
      if (sortColumn === 'date' && viewType === 'daily') {
  ...
  }, [data, sortColumn, sortDirection, viewType]);
  ...
  const handleRunReport = () => {
    ...
    // Reset sort when running a new report
    setSortColumn('');
    setSortDirection('asc');
          ...
          <div className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
            <div className="flex flex-wrap gap-6 justify-between">
              <div className="flex gap-6">
                ...
                <div className="flex items-center self-center space-x-6 mt-8 -mt-30">
                      ...
                      <ul className="py-1">
                        ...
                        <li className="px-3 py-2 border-b">
                          <label className="flex items-center">
                            <input
                              type="checkbox"
                              checked={visibleColumns.occupiedRooms}
                              onChange={() => setVisibleColumns({...visibleColumns, occupiedRooms: !visibleColumns.occupiedRooms})}
                              className="mr-2"
                            />
                            Occupied Rooms
                          </label>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-gray-100">
                    {visibleColumns.date && (
                      <th 
                        className="text-left py-2 px-4 border-b border-gray-300 cursor-pointer hover:bg-gray-200"
                        onClick={() => handleSort(viewType === 'daily' ? 'date' : 'period')}
                      >
                        <div className="flex items-center">
                          {viewType === 'daily' ? 'Date' : 'Period'}
                          {sortColumn === (viewType === 'daily' ? 'date' : 'period') && (
                            <svg 
                    ...
                    {visibleColumns.soldRooms && (
                      <th 
                        className="text-left py-2 px-4 border-b border-gray-300 cursor-pointer hover:bg-gray-200"
                        onClick={() => handleSort('soldRooms')}
                      >
                        <div className="flex items-center">
                          Occupied Rooms
                          {sortColumn === 'soldRooms' && (
                            <svg 
                              className={`w-4 h-4 ml-1 ${sortDirection === 'desc' ? 'transform rotate-180' : ''}`}
                              fill="currentColor" 
                              viewBox="0 0 20 20"
                    ...
                    {visibleColumns.occupancyPercentage && (
                      <th 
                        className="text-left py-2 px-4 border-b border-gray-300 cursor-pointer hover:bg-gray-200"
                        onClick={() => handleSort('occupancyPercentage')}
                      >
                        <div className="flex items-center">
                          Occupancy %
                          {sortColumn === 'occupancyPercentage' && (
                            <svg 
                              className={`w-4 h-4 ml-1 ${sortDirection === 'desc' ? 'transform rotate-180' : ''}`}
                              fill="currentColor" 
                              viewBox="0 0 20 20"
                    ...
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      {visibleColumns.guests && (
                        <td className="py-2 px-4 border-b border-gray-200">
                          {viewType === 'daily' ? (
                            <button 
                              onClick={() => setSelectedGuestList(item.date)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-4/5 max-w-4xl max-h-[80vh] flex flex-col">
            ...
            <div className="overflow-auto p-6 flex-grow">
              {data.find(item => item.date === selectedGuestList)?.guestDetails.length > 0 ? (
                <table className="w-full border-collapse text-sm">
                  ...
                  <tbody>
                    {data.find(item => item.date === selectedGuestList)?.guestDetails.map(guest => (
                ...
                <div className="text-center py-8 text-gray-500">No guest details available for this date.</div>
              )}
            </div>
            <div className="border-t px-6 py-4 flex justify-between items-center text-sm">
              <div>
                Total Guests: {data.find(item => item.date === selectedGuestList)?.guests || 0}
              </div>
              <button 
